...
def kmeans_filter(img):
    """Filter colors to transform image to grayscale using KMeans(2)"""

    colors = img.reshape(-1, 1)
    kmeans = MiniBatchKMeans(2)
    kmeans.fit(colors)
    if kmeans.cluster_centers_[0] > kmeans.cluster_centers_[1]:
        # cluster 1 is text -> inverse background to white
        new_colors = np.array(255 - kmeans.predict(colors) * 255, dtype=np.uint8)
    else:
        # cluster 1 is background which is already white
        new_colors = np.array(kmeans.predict(colors) * 255, dtype=np.uint8)
...
def text_standardize(text):
    ...
    return text
def preprosess_raw(img_path, input_size, save_type=False):

    if isinstance(img_path, str):
        img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)

    if isinstance(img_path, tuple):
        image, boundbox = img_path
    ...
    if isinstance(save_type, str):
        ...
        cv2.imwrite(os.path.join('..', 'data', save_type, file_name), img)
    return img